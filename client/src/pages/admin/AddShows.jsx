  ...
  return nowPlayingMovies.length > 0 ? (
    <>
     ...
     <div className="mt-8">
          ...
          px-3 py-2 rounded-md w-[200px]">
            <p className="text-gray-400 text-sm">{currency}</p>
            <input min={0} type="number" value={showPrice} className="outline-none"
            placeholder="Enter Show Price" onChange={(e) => setShowPrice(e.target.value)}/>
          </div>
     </div>

     {/* Date and Time Input*/}
     <div className="mt-6">
          <label className="text-sm font-medium mb-2 block">Select Date and Time</label>
          ...
          rounded-lg">
            ...
            text-sm rounded-lg hover:bg-primary cursor-pointer px-3 py-2">
              Add Time
            </button>
          </div>
     </div>
